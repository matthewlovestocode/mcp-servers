...
async function main(): Promise<void> {
  ...
  const server = createGitHubServer(config);
  ...
  console.error("gh-mcp server ready.");

  const shutdown = async (signal: NodeJS.Signals) => {
    console.error(`Received ${signal}. Shutting down gh-mcp server...`);
...
main().catch((error: unknown) => {
  if (error instanceof Error) {
    ...
    console.error("Unexpected error starting gh-mcp server:", error);
  }
  process.exit(1);
});
